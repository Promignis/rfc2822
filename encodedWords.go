...
func decodeToUTF8Base64Header(input string) string {
	...
	//
	// issue stated in enmime#112, but only in the case of a quoted display-name
...
func decodeHeader(input string) string {
	...
	// By default mime.WordDecoder already handles utf-8, iso-8859-1 and us-ascii
	// so a nil CharsetReader will also work
